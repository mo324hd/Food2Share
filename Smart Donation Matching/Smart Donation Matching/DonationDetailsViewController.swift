...
class DonationDetailsViewController: UIViewController {
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if segue.identifier == "showCollectorsSegue", let destinationVC = segue.destination as? ViewController, let donorCoordinate = sender as? CLLocationCoordinate2D
        {
...
extension DonationDetailsViewController: UIPickerViewDelegate, UIPickerViewDataSource
{
...
extension DonationDetailsViewController: UITextFieldDelegate, UITextViewDelegate
{
