...
// Scene delegate sets up the window and shows the EmergencyDonationViewController as root.
class PrimarySceneDelegate: UIResponder, UIWindowSceneDelegate {
    var window: UIWindow?
...
final class EmergencyDonationViewController: UIViewController {
    ...
    private func setupLayout() {
        ...
        var backConfig2 = UIButton.Configuration.plain()
        backConfig2.image = UIImage(systemName: "chevron.left")
        backConfig2.title = "Back"
        backConfig2.imagePadding = 6
        backConfig2.baseForegroundColor = .label
        backButton.configuration = backConfig2
        backButton.addTarget(self, action: #selector(headerBackTapped), for: .touchUpInside)
        backRow.addSubview(backButton)
        backButton.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([
            backButton.leadingAnchor.constraint(equalTo: backRow.leadingAnchor, constant: 12),
            backButton.topAnchor.constraint(equalTo: backRow.topAnchor, constant: 8),
            backButton.bottomAnchor.constraint(equalTo: backRow.bottomAnchor, constant: -8)
        ])
        ...
        headerContainer.heightAnchor.constraint(equalToConstant: 76).isActive = true
        ...
        headerView.heightAnchor.constraint(equalToConstant: 76).isActive = true
        ...
        donationAmountTitle.font = UIFont.systemFont(ofSize: 18, weight: .semibold)
        ...
        currencyBadge.widthAnchor.constraint(equalToConstant: 44).isActive = true
        ...
        dateTitle.font = UIFont.systemFont(ofSize: 18, weight: .semibold)
        ...
        donateButton.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: .semibold)
    ...
    @objc private func donateNow() {
        ...
        _ = "You are donating $\(String(format: "%.2f", validAmount)) on \(dateString) via \(selectedPayment)."
    ...
    @objc private func headerBackTapped() {
        if let nav = navigationController {
            if nav.viewControllers.first === self {
                // If this is root, try to dismiss if presented
...
final class CreditCardPaymentViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        summaryHeader.font = UIFont.systemFont(ofSize: 16, weight: .semibold)

        let details = UILabel()
        details.numberOfLines = 0
        details.textColor = .secondaryLabel
        details.text = "Amount: $\(String(format: "%.2f", amount))\nDate: \(dateString)"
        ...
        actionsHeader.font = UIFont.systemFont(ofSize: 16, weight: .semibold)
        ...
        NSLayoutConstraint.activate([
            ...
            scroll.topAnchor.constraint(equalTo: backButton.bottomAnchor, constant: 8),
...
final class PayPalPaymentViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let stack = UIStackView(arrangedSubviews: [titleLabel, summaryHeader, summaryCard, actionsHeader, payPalButton])
        stack.axis = .vertical
        stack.spacing = 20
        stack.translatesAutoresizingMaskIntoConstraints = false
